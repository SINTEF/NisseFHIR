...
### Unit Tests (58 tests in `server/src/`)
...
- `validation::tests` — Schema validator plus secondary datatype checks: accepts minimal and named Patient, minimal Observation with any code string for status, integer boundary values, rejects unknown resource types, additional properties, wrong types, invalid calendar dates/dateTimes, fractional or out-of-range integer fields, invalid positiveInt/unsignedInt values, and overflowing integer64 strings. Validator caching and multi-type support.

### Integration Tests (104 tests in `server/tests/`)
...
- Broader FHIR datatype and invariant coverage beyond the current second-pass slice, especially decimal precision and range checks, URI/canonical/url semantic validation, and complex datatype invariants such as `Period.start <= end`, `Quantity.code -> system`, and `Attachment.data -> contentType`.
