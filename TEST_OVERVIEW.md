...
- `graceful_shutdown.rs` (runtime test) — Spawns the real server binary, waits for it to accept connections, sends `SIGTERM`, and asserts the process exits cleanly within the bounded grace window.
