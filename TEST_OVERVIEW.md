...
- `capability::tests` — Capability statement shape and contract: stable canonical URL, deployment-specific implementation URL, concrete resource types, exact executable search-registry coverage, supported interactions, truthful JWT security metadata, package-derived version, omitted drifting release dates, JSON-only formats, conditional create, and update-as-create.
...
### Integration Tests (244 tests in `tests/`)
...
- `crud.rs` — Create returns 201 with correct headers (ETag, Last-Modified, Location), ignores submitted IDs in favor of server-generated IDs, uses insert-only atomic persistence, database persistence, initial version=1, read-after-create roundtrip, ETag/Last-Modified on read, 404 for nonexistent/wrong-type, update and update-as-create behavior, optional `If-Match` compatibility, stale `If-Match` returns 412, multi-resource-type roundtrip, field preservation on update, healthz, the standard and legacy metadata routes, and a contract check matching advertised interactions to executable routes.
...
- `graceful_shutdown.rs` (runtime test) — Spawns the real server binary, waits for it to accept connections, sends `SIGTERM`, and asserts the process exits cleanly within the bounded grace window. Also covers: metrics enabled (both the main and telemetry listeners accept, `/metrics` serves a Prometheus exposition, and SIGTERM drains both cleanly) and metrics disabled (no telemetry listener opens).
- `metrics.rs` — Prometheus telemetry end-to-end: `/metrics` serves the `text/plain; version=0.0.4` exposition with all six required application families plus `process_*` series; successful/failing/unmatched requests produce normalized route-template labels; in-flight gauge returns to zero; database pool gauges sum to the current pool size and are re-sampled (not accumulated) across repeated scrapes; concrete FHIR IDs and query values never leak into the output; non-`/metrics` telemetry paths return 404.
